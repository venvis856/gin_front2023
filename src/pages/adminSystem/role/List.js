...
import {listApi, deleteApi} from "../../../server/role";
...
function List(props) {
    ...
    const columns = [
        ...
        {title: '角色名', dataIndex: 'role_name', width: 80, align: 'center'},
        {
        ...
        },
        {
            ...
            render: (txt, record) => {
                if (txt === 1) {
                    return <span style={{color:'#16ba39'}}>全局角色</span>
                } else if (txt === 2) {
                    return <span style={{color:'#000'}}>超级管理员</span>
                }else if (txt === 3) {
                    return <span style={{color:'#000'}}>普通员工</span>
                }
            }
        },
        {
            ...
            render: (txt, record, index) => { //txt 根据上面dataIndex确定具体该行的某个值, record 整条, index 从0开始的下标
                return (
                    <div>
                        <Button type="primary" shape="round" size="small" style={{marginRight:'5px'}} onClick={() => {
                            setEditInfo(record)
                            setEditVisible(true)
                        ...
                        <Button type="primary" shape="round" size="small" onClick={() => {
                            ...
                            setPermissionInfo(record)
                            setPermissionVisible(true)
                        }}>设置权限</Button>
                        <Popconfirm title="确定删除此项?" onCancel={() => console.log('取消删除')}
                                    onConfirm={() => {
                                        console.log("确认删除", txt, record, index)
                                        //此处调用api接口进行操作
                                        setloading(true)
    ...
    const [search, setSearch] = useState({
        ...
        role_name: {operator: 'like', value: '', type: 'both'},
    ...
    return (
        <React.Fragment>
            <div className="search_body">
                ...
                <div className="search_item">
                    <div className="search_item_font">角色名:</div>
                    <Input className="search_item_input" allowClear placeholder="请输入"
                           onChange={(e) => handleChangeInput(e, 'role_name')}/>
                </div>
                <div className="search_item">
            ...
            <OperatePermission info={permissionInfo} visible={permissionVisible} onClose={()=>setPermissionVisible(false)} />
        </React.Fragment>
    )
}
